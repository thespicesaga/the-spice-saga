...
app.post("/api/create-order",async(req,res)=>{try{if(!rp)return res.status(503).json({error:"Razorpay is not connected yet."});let{amount,name,phone,location,notes,items}=req.body;if(!Number.isFinite(amount)||amount<=0||!name||!/^[0-9]{10}$/.test(phone)||!location||!items?.length)return res.status(400).json({error:"Please enter valid order details."});let receipt="SS-"+Date.now().toString().slice(-10);let o=await rp.orders.create({amount:Math.round(amount*100),currency:"INR",receipt,capture:true,notes:{restaurant:"The Spice Saga"}});let orders=read();orders.push({order_number:receipt,razorpay_order_id:o.id,status:"payment_pending",amount,name,phone,location,notes,items,created_at:new Date().toISOString()});save(orders);res.json({id:o.id,amount:o.amount,key_id:process.env.RAZORPAY_KEY_ID})}catch(e){res.status(500).json({error:e.error?.description||e.message})}});
app.post("/api/verify-payment",(req,res)=>{try{let{razorpay_order_id,razorpay_payment_id,razorpay_signature}=req.body,orders=read(),o=orders.find(x=>x.razorpay_order_id===razorpay_order_id);if(!o)return res.status(404).json({success:false,error:"Order not found"});let expected=crypto.createHmac("sha256",process.env.RAZORPAY_KEY_SECRET).update(razorpay_order_id+"|"+razorpay_payment_id).digest("hex"),a=Buffer.from(expected),b=Buffer.from(razorpay_signature||"");if(a.length!==b.length||!crypto.timingSafeEqual(a,b))return res.status(400).json({success:false,error:"Invalid payment signature"});o.status="paid";o.razorpay_payment_id=razorpay_payment_id;o.paid_at=new Date().toISOString();save(orders);res.json({success:true,order_number:o.order_number})}catch(e){res.status(500).json({success:false,error:e.message})}});
app.get("/api/admin/orders",(req,res)=>{if(req.headers["x-admin-password"]!==process.env.ADMIN_PASSWORD)return res.status(401).json({error:"Unauthorized"});res.json(read().reverse())});
app.patch("/api/admin/orders/:id",(req,res)=>{if(req.headers["x-admin-password"]!==process.env.ADMIN_PASSWORD)return res.status(401).json({error:"Unauthorized"});let allowed=["paid","accepted","preparing","ready","delivered","cancelled"],orders=read(),o=orders.find(x=>x.order_number===req.params.id);if(!o||!allowed.includes(req.body.status))return res.status(400).json({error:"Invalid order/status"});o.status=req.body.status;o.updated_at=new Date().toISOString();save(orders);res.json(o)});
app.listen(PORT,()=>console.log("The Spice Saga running on port "+PORT));
